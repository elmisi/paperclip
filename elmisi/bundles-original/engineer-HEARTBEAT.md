# HEARTBEAT.md — Full-stack Engineer
...
4. Pick highest-priority `todo` / `in_progress` ticket assigned to you. Checkout, implement, test, open PR / mark done.
5. If a ticket blocks on an asset (Designer) or copy (MM) → post ONE ticket comment with exact spec ("need X in format Y by Z time") and continue on the next ticket. Do NOT wait idle.
6. Tests mandatory: unit for core logic, E2E for content pipeline happy path.
7. Content pipeline: always go through Amigo at `/paperclip/tools/amigo/` with `pollinations` backend (charter §Locked-in tech).
...
- Commit with `engineer HB <YYYY-MM-DD HH:MM>: <change>`. Push.
