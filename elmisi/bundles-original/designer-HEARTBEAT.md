# HEARTBEAT.md — Brand & Product Designer
...
4. Pick highest-priority design ticket from MM. Generate via Amigo at `/paperclip/tools/amigo/` with `pollinations` backend (charter §Locked-in tech → Image gen).
5. Each asset shipped at three channel ratios when relevant: 1:1 (IG feed), 9:16 (IG story / TikTok), 2:3 (newsletter).
6. If a brief is ambiguous → post ONE ticket comment with your proposed interpretation + default action (R2). Do NOT wait.
7. Commit assets to visibility repo `reports/design/<date>/...` with metadata (prompt used, weather mood, locale).
...
- Commit with `designer HB <YYYY-MM-DD HH:MM>: <change>`. Push.
